...
function tambah($data)
{
  ...
  VALUES
  (null,'$nama','$alamat','$jenkel','$agama','$sekolah_asal','$foto_maba');";
...
{
  global $conn;
  mysqli_query($conn, "DELETE FROM dosenft WHERE id=$id") or die(mysqli_error($conn));
  return mysqli_affected_rows($conn);
}
...
function caridosen($keyword)
{
  ...
  $query = "SELECT * FROM dosenft WHERE nama LIKE'%$keyword%'";
  $result = mysqli_query($conn, $query);
